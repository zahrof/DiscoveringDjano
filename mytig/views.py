...
class RedirectionPointsDeLivraison(APIView):
    def get(self,request,format=None):
        response = requests.get(baseUrl+'shipPoints/')
        jsondata = response.json()
        return Response(jsondata)

class RedirectionDetailPointLivraison(APIView):
    def get_object(self, pk):
        try:
...
class ProduitsDispo(APIView):
    def get(self, request, formet=None):
        res=[]
        for prod in ProduitsDispos.objects.all():
            serializer = ProduitsDispoSerializer(prod)
            response = requests.get(baseUrl+'product/'+str(serializer.data['tigID'])+'/')
            jsondata = response.json()
            res.append(jsondata)
        return JsonResponse(res, safe=False)